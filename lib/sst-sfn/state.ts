...
function isChainable(obj: unknown): obj is Chainable {
  return typeof obj === "object" && obj !== null && "name" in obj && "next" in obj;
}
...
export class StateBase implements Chainable {
  ...
  addCatch(c: CatchProps | Chainable) {
    ...
    this._catches.push(isChainable(c) ? { ErrorEquals: [TaskState.ALL], Next: c } : c);
    return this;
  }
  ...
  toJSON(dontEnd: boolean = false) {
    return {
      ...(this._nextName ? { Next: this._nextName } : !dontEnd ? { End: true } : {}),
      Retry: this._retries,
